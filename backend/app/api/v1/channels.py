...
async def save_channel_config(
    ...
    from app.crud.channel_config import crud_channel_config
    ...
    cfg_types = ["hot_questions", "banners", "quick_tags"]
    for cfg_type in cfg_types:
        key = cfg_type
        items = data.get(key, [])
        await crud_channel_config.upsert(db, channel_id, key, items)
...
async def get_channel_config_public(
    ...
    configs = await crud_channel_config.get_by_channel(db, channel_id)
    ...
    for cfg in configs:
        if cfg.status == 0:
            continue
        if cfg.config_type == "hot_questions":
            result["hot_questions"].append(
                {
                    "text": cfg.title or "",
                    "send_text": cfg.content or "",
                }
            )
        elif cfg.config_type == "banners":
            result["banners"].append(
                {
                    "title": cfg.title or "",
                    "subtitle": cfg.content or "",
                    "image_url": cfg.image_url,
                    "link_url": cfg.link_url,
                }
            )
        elif cfg.config_type == "quick_tags":
            result["quick_tags"].append(
                {
                    "icon": extra.get("icon", ""),
                    "label": row.title or "",
                    "send_text": row.content or "",
